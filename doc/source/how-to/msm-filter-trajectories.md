...
Strip waters (or any other atom selection) out of a simlist's trajectories *before* they hit the Metric / TICA / clustering pipeline. Filtered trajectories are written to disk as a new XTC set, which cuts per-frame I/O and memory use for downstream analysis considerably. This is the same step the adaptive-sampling pipeline performs automatically on completed sims.
