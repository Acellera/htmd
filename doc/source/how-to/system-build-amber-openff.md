...
- **ForceField-XML form**: `structure.cif` + `structure.pdb` (when it fits) + one `structure.smallmol_<i>.xml` per OpenFF/SMIRNOFF-templated ligand + any `extra_xml` files copied verbatim + `system.yaml` (a manifest listing the structure, the ordered parameter set, and the box). This is the form ACEMD reads natively when you pass `system.yaml` to it: no prmtop round-trip through ParmEd's atom-type bucketing, so parameters stay byte-exact as OpenMM built them.
...
`builder.build` returns a 2-tuple: the rebuilt `Molecule` (with bonds / charges from the OpenMM `ForceField`) and the parameterized OpenMM `System` object. The protein is parameterized with the default AMBER14 XML stack (protein.ff14SB + lipid17 + nucleic + tip3p, from {py:func}`~htmd.builder.openmm.defaultFf`); the ligand is matched against `mol`'s atoms and parameterized on-the-fly via the OpenFF SMIRNOFF template generator. The output `./build/structure.prmtop` is loadable by ACEMD (via the bundled `system.yaml` config) and by OpenMM (`app.AmberPrmtopFile`).

## Parameters that matter

| Parameter | What it does |
| --- | --- |
| `ff` | OpenMM XML force field files for the protein / lipid / nucleic / water stack. Default is {py:func}`~htmd.builder.openmm.defaultFf` = `["amber14/protein.ff14SB.xml", "amber14/lipid17.xml", "amber14/DNA.bsc1.xml", "amber14/RNA.OL3.xml", "amber14/tip3p.xml"]`. Replace, don't append: passing `ff=` overrides the full default. |
...
- Sage **2.3** (`openff-2.3.0`) assigns partial charges through a pretrained graph-neural-network surrogate (NAGL), skipping the conformer / AM1-BCC step, so the first build on a new ligand is fast (sub-second per molecule). Older versions (`openff-2.0` / `2.1` / `2.2`) and GAFF (`gaff-2.2.20`) still run real AM1-BCC and take a few seconds per unique ligand. Caching is in-memory per process only; separate Python runs always re-evaluate charges.
