...
The plot shows one curve per slow process, plotting implied timescale **against Markov lag time**. The X axis is the lag time you'd pass to `model.markovModel(lag, ...)`; the Y axis is the timescale that lag-time would imply.
...
Pick the **shortest lag at which the top curves have plateaued**, which maximises kinetic resolution without breaking Markovianity. Pick **macronum = (number of distinct slow timescales) + 1**.
...
- **Plateau region**: every curve climbs sharply from lag ≈ 0 then flattens out by lag ≈ 4 ns. Pick the **shortest** plateau lag, which is ≈ 4 ns here.
