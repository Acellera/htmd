...
| `samplemode` | For `statetype="macro"` one of `"weighted"` (default; proportional to microstate eq populations, and the best choice for "representative" frames), `"even"` (uniform across all microstates of the macrostate), or `"random"`. For `statetype="micro"` / `"cluster"` only `"random"` is meaningful. |
...
- {doc}`How to read off and interpret an ITS plot <msm-interpret-its-plot>`: how you chose `macronum` in the first place.
