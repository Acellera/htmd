...
| --- | --- |
| `app` | The {py:class}`~jobqueues.simqueue.SimQueue` that runs the actual ACEMD jobs. Local GPU, Slurm, LSF, and PBS all share the same interface. |
...
| `projection` | The feature space adaptive clusters on: a moleculekit `Metric*`, or a list of them. |
...
Useful when you have a large GPU cluster and per-sim cost is low, throwing many short trajectories at the problem.
