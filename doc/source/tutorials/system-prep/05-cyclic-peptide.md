...
- For peptides whose ring closes through a *side chain* (lactam bridges, isopeptide cycles, thioether cycles), detection emits a {py:class}`~moleculekit.tools.nonstandard_residues.ChainResidueSpec` with the appropriate anchor atom and the cycle closure is wired automatically, the same pattern the {doc}`stapled-peptide tutorial <04-stapled-peptide>` shows.

## See also

- {doc}`Build a stapled peptide <04-stapled-peptide>`: a chemical crosslink between two NCAAs at non-backbone atoms.
