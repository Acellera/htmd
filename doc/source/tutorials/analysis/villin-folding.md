...
3. **Reduce dimensions** with TICA to find the slow collective coordinates.
...
## Step 3: TICA, to find the slow coordinates
...
Flux pathways visualise the dominant transition routes in TPT (transition-path theory) form, showing which intermediates the protein actually visits on its way from unfolded to folded.
...
| `model.markovModel(25, 4, units="ns")` lag | Read off the ITS plot: the shortest lag at which slow timescales plateau. |
