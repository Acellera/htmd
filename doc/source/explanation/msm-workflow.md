...
A **projection** maps each frame of each simulation to a feature vector. Projections come from moleculekit's `metric*` family ({py:class}`~moleculekit.projections.metricdistance.MetricDistance`, {py:class}`~moleculekit.projections.metricrmsd.MetricRmsd`, {py:class}`~moleculekit.projections.metricdihedral.MetricDihedral`, {py:class}`~moleculekit.projections.metricsecondarystructure.MetricSecondaryStructure`, and so on), configured once and applied across the whole `SimList`:
