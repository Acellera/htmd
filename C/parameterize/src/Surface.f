...
C     INSERT VAN DER WAAL RADII FOR MN
      VANDER(25)=1.10D0

C     INSERT VAN DER WAAL RADII FOR LA
      VANDER(57)=1.20D0
...
C The next line is commented out by Lei. 
C            if(ITYPE .ne. 10) then
...
C               DO K = 2, Exclude(1)+1
C                 PERTURBATION point - "Exclude(K)" atom
C                  IF(DIST2(TEMP,COORD(1,Exclude(K))).LT.RRATMS) GOTO 100
C               ENDDO
C            endif
C
