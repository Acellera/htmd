...
void CFitting::To_Detect_Invalid_Acceptor_Donor(void)
{
	...
	for(i=0; i<N_Donor; i++)	{
		if( fabs(r_Min_MM_Donor[i] - r_Min_QM_Donor[i]) > 0.7 )	{	// too large distance
			ToFit_Donor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 1)\n", Donor_List_Active[i]+1);
		}
		else if( r_Min_QM_Donor[i] > (Upper_Bound_R_H_Bond + 0.2) )	{
			ToFit_Donor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 2)\n", Donor_List_Active[i]+1);
		}
		else if( r_Min_MM_Donor[i] > (Upper_Bound_R_H_Bond - 0.01) )	{
			ToFit_Donor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 3)\n", Donor_List_Active[i]+1);
		}
		else	{
			ToFit_Donor[i] = 1;
			nFit_Donor++;
	...
	for(i=0; i<N_Acceptor; i++)	{
		if( fabs(r_Min_MM_Acceptor[i] - r_Min_QM_Acceptor[i]) > 0.7 )	{	// too large distance
			ToFit_Acceptor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 4)\n", Acceptor_List_Acceptor[i]+1);
		}
		else if( r_Min_QM_Acceptor[i] > (Upper_Bound_R_H_Bond + 0.2) )	{
			ToFit_Donor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 5)\n", Donor_List_Active[i]+1);
		}
		else if( r_Min_MM_Acceptor[i] > (Upper_Bound_R_H_Bond - 0.01) )	{
			ToFit_Donor[i] = 0;
			fprintf(fFile_Run_Log, "Warning> The interaction of donor atom %d and water is not going to be fitted. (case 6)\n", Donor_List_Active[i]+1);
		}
		else	{
			ToFit_Acceptor[i] = 1;
			nFit_Acceptor++;
