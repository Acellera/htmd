...
// MJH begin
#if 0 // Write out an XYZ file
		sprintf(szNameGjf, "qm-scan-%d-%d.xyz", Active_Phi+1, IdxScan+1);
  	fOut = fopen(szNameGjf, "w");
		fprintf(fOut, "%d\n\n", nAtom );
		for(i=0; i<nAtom; i++)	{
			fprintf(fOut, "%s  %9.5lf %9.5lf %9.5lf\n", szElemName[i], Mol.x[i], Mol.y[i], Mol.z[i]);
...
//			if(FindString(szLine, "     Input orientation:")>=0)	{	// to extract the coordinate
			if(FindString(szLine, " orientation:")>=0)	{	// to extract the coordinate
				...
				for(i=0; i<nAtom; i++)	{
					...
					printf("Read: %d %f %f %f\n", i, Mol.x[i], Mol.y[i], Mol.z[i] );
					if(ReadItem != 3)	{
						printf("ERROR: Didn't read natoms coords\n" );
						ToRead = 0;
						break;
					}
				}
			...
			if( energyset && coordsset && optcompleted ) { 
...
#if 0 
//-- ORIGINAL CODE USING GAUSSIAN DIH SCAN
	for(; i<=n_Bins; i++)	{
		if( Flag_To_Scan[i] == 0 )	{
