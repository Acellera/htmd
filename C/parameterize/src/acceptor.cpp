...
//							GenerateWater();
			...
			RestoreDummyAtoms();
			fprintf(fFile_Run_Log, "mol_H_acceptor.cpp 1> %2d H acceptor: %2d atom %3s MM, E_min = %7.4lf r_min = %7.4lf\n", 
...
void Extract_All_E_Dimer(void)
{
    ...
    printf( "njob %d : QM energy %f  dimer far %f : delta %f\n", i, E_Min, E_Dimer_Far, E_Min_QM );
		...
		fprintf(fFile_Run_Log, "mol_H_acceptor.cpp 2> %2d H acceptor: %2d atom %3s QM, E_min = %7.4lf r_min = %7.4lf\n", 
...
void Output_Optimize_Water_Pose_QM(void)
{
	FILE *fOut;
	int i;
	char szName_Result[]="qm.out", szAcceptor[256];
//	chdir( szCurDir );
	...
	int h1w= n_Atom_In_Mol + 3;
	int x3 = n_Atom_In_Mol + 4;
	int o1w= n_Atom_In_Mol + 5;
	int h2w= n_Atom_In_Mol + 6;

	fprintf(fOut, "%s   %10.5lf%10.5lf%10.5lf\n", "X", x_Dummy_1, y_Dummy_1, z_Dummy_1);
	fprintf(fOut, "%s   %10.5lf%10.5lf%10.5lf\n", "X", x_Dummy_2, y_Dummy_2, z_Dummy_2);

	fprintf(fOut, "H  %d  roh %d 90.0 %d 180.0 \n", ActiveAtom+1, x2, x1);

	fprintf(fOut, "X   %d  %10.4lf %d  90.0 %d   0.0 \n",  h1w, R_DUMMY_2, ActiveAtom+1, x2);
	fprintf(fOut, "O   %d  %10.4lf %d  90.0 %d 180.0 \n", h1w, b0_H_O_HF, x3, x2);
