...
class _TestParameterize(unittest.TestCase):
    ...
    def _test(
        ...
        refDir,
        resDir,
        energyTermRelTol=1e-6,
        energyProfileAbsTol=1e-6,
        ...
        for root, _, files in os.walk(refDir, followlinks=True):
            for file in files:
                ...
                if any(
                    [
                        relFile.startswith(exclusion) or relFile.endswith(exclusion)
        ...
        for file in testedFiles:
            ...
            with self.subTest(refFile=refFile):
                ...
                self.assertTrue(
                    os.path.exists(resFile),
                    msg="Could not find file {} corresponding to {}".format(
                        resFile, refFile
                    ),
                )
                ...
                if (
                    file.endswith("frcmod")
                    or file.endswith("rtf")
                    or file.endswith("prm")
                ):
                    refLines = refLines[1:]
                    resLines = resLines[1:]
                ...
                self.assertEqual(
                    len(refLines),
                    len(resLines),
                    msg="Reference file {} has a different number of lines ({}) from "
                    "file {} ({})".format(
                        refFile, len(refLines), resFile, len(resLines)
                    ),
                )
                ...
                for refLine, resLine in zip(refLines, resLines):
                    ...
                    for iField, (refField, resField) in enumerate(
                        zip(refFields, resFields)
                    ):
                        ...
                        if relFile.endswith(".frcmod"):
                            if (
                                ...
                                len(refFields) == 7 and iField == 3
                            ):  # Detect dihedral phase column
                        ...
                        self.assertListEqual(
                            refLines, resLines
                        )  # If there is a mismatch, print a diff of all file
    ...
    def test_h2o2_gaff2(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 -f GAFF2 --charge-type Gasteiger --min-type None --no-dihed",
        )
        self._test(refDir, resDir)
    ...
    def test_h2o2_min(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 -f GAFF2 --min-type qm --charge-type Gasteiger "
    ...
    def test_h2o2_min_restart(self):
        ...
        shutil.copytree(
            os.path.join(refDir, "minimize"), os.path.join(resDir, "minimize")
        )
        self._run(
            refDir,
            resDir,
    ...
    def test_h2o2_esp(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --min-type None --charge-type ESP "
            "--no-dihed --theory B3LYP --basis cc-pVDZ",
        )
        self._test(refDir, resDir)
    ...
    def test_h2o2_dihed_fix(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --min-type None --charge-type Gasteiger "
    ...
    def test_h2o2_dihed_fix_restart(self):
        ...
        shutil.copytree(
            os.path.join(refDir, "dihedral-single-point"),
    ...
    def test_h2o2_dihed_opt_restart(self):
        ...
        shutil.copytree(
            os.path.join(refDir, "dihedral-opt"), os.path.join(resDir, "dihedral-opt")
        )
        self._run(
            refDir,
            resDir,
        ...
        self._test(
            refDir,
            resDir,
            energyTermRelTol=1e-5,
            energyProfileAbsTol=1.1e-3,
            dihedralForceConstAbsTol=1e-5,
            dihedralPhaseAbsTol=1.1e-3,
        )
    ...
    def test_h2o2_full_fake(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --min-type qm --charge-type ESP "
    ...
    def test_ethene_dihed_fix_restart(self):
        ...
        self._test(
            refDir,
            resDir,
            energyTermRelTol=5e-6,
            dihedralForceConstAbsTol=5e-6,
            dihedralPhaseAbsTol=1.1e-3,
        )
    ...
    def test_glycol_dihed_fix_restart(self):
        ...
        self._test(
            refDir,
            resDir,
            energyTermRelTol=3e-5,
            energyProfileAbsTol=1.1e-3,
            dihedralForceConstAbsTol=1e-4,
            dihedralPhaseAbsTol=0.6,
        )
    ...
    def test_ethanolamine_dihed_fix_restart(self):
        ...
        self._test(
            refDir,
            resDir,
            energyTermRelTol=2e-4,
            energyProfileAbsTol=5.1e-3,
            dihedralForceConstAbsTol=5e-4,
            dihedralPhaseAbsTol=3.5,
        )
    ...
    def test_benzamidine_gasteiger(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --min-type None --charge-type Gasteiger "
            "--no-dihed",
        )
        self._test(refDir, resDir)
    ...
    def test_benzamidine_am1_bcc(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --min-type None --charge-type AM1-BCC "
            "--no-dihed",
        )
        self._test(refDir, resDir)
    ...
    def test_benzamidine_gaff(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --forcefield GAFF --min-type None "
    ...
    def test_benzamidine_gaff2(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --forcefield GAFF2 --min-type None "
            "--charge-type Gasteiger --no-dihed",
        )
        self._test(refDir, resDir)
    ...
    def test_benzamidine_cgenff(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --forcefield CGenFF_2b6 --min-type None "
            "--charge-type Gasteiger --no-dihed",
        )
        self._test(refDir, resDir)
    ...
    def test_benzamidine_rtf_prm(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --forcefield CGenFF_2b6 "
    ...
    def test_benzamidine_full(self):
        assert (
            "HTMD_CONFIG" in os.environ
        ), '"HTMD_CONFIG" environment variable has to be set'
        refDir = os.path.join(self.dataDir, "benzamidine_full")
        resDir = os.path.join(self.testDir, "benzamidine_full")
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --charge 1 --forcefield GAFF2 --min-type qm "
    ...
    def test_h2o2_dihed_opt_mm_fake(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --forcefield GAFF2 --min-type mm "
    ...
    def test_glycol_dihed_opt_mm_fake(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --forcefield GAFF2 --min-type None "
    ...
    def test_water_full(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --min-type qm --charge-type ESP --theory B3LYP --basis cc-pVDZ",
        )
        self._test(refDir, resDir)
    ...
    def test_water_min_mm_charge_bcc(self):
        ...
        self._run(
            refDir,
            resDir,
            "parameterize input.mol2 --min-type mm --charge-type AM1-BCC",
        )
        self._test(refDir, resDir)
    ...
    def test_water_min_mm_charge_none(self):
        ...
        self._run(
            refDir, resDir, "parameterize input.mol2 --min-type mm --charge-type None"
        )
        self._test(refDir, resDir)
