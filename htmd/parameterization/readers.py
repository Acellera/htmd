...
def readRTF(filename):
    ...
    for idx in range(len(names)):
        ...
        if atype not in element_by_type:
            ...
            logger.info(
                "Guessing element %s for atom %s type %s"
    ...
    element_by_idx = np.array(
        [element_by_type[t].lower().capitalize() for t in type_by_index], dtype=object
    )
    ...
    for type_ in type_by_index:
        if re.match(_ATOM_TYPE_REG_EX, type_):
            raise ValueError(
                'Atom type %s is incompatible. It cannot finish with "x" + number!'.format(
                    type_
                )
            )
...
def readPREPI(mol, prepi):
    ...
    if impropers:
        ...
        for improper in impropers:
            idx = [
                index_by_name[name.upper()] for name in improper
            ]  # conv atom name to indices
...
def readFRCMOD(atomtypes, frcmod):
    ...
    mass_by_atomtype = {
        line.split()[0]: float(line.split()[1]) for line in section.group(1).split("\n")
    }
    element_by_atomtype = {}
    for at in mass_by_atomtype:
        element_by_atomtype[at] = elements_from_masses(mass_by_atomtype[at])

    element_by_idx = np.array([element_by_atomtype[at] for at in atomtypes]).astype(
        np.object
    )
    mass_by_idx = np.array([mass_by_atomtype[at] for at in atomtypes]).astype(
        np.float32
    )
