...
log = logging.getLogger("htmd.parameterization")
    ...
    def run(self, rootdir, param=None):
        ...
        for i in self._steps:
            ...
            if not i.completed(directory):
                ...
                if not i.completed(directory):
                    raise ValueError("run(): Step [" + i.name + "] did not produce the expected output: " + str(
                        i.missing(directory)) + " in directory " + directory)
