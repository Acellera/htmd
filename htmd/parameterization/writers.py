...
def getSortedAndUniqueTypes(types, field):
    ...
    if field == "bond_types":
        ...
        for i in range(types.shape[0]):
            types[i][0], types[i][1], types[i][3] = sorted(
                [types[i][0], types[i][1], types[i][3]]
            )
    else:
...
def writeFRCMOD(mol, parameters, filename, typemap=None):
    ...
    for type in types:
        ...
        if field == "improper_periodic_types":
            ...
            f.write(
                fmt.format(
                    type[0], type[1], type[2], type[3], val.phi_k, val.phase, val.per
                )
            )
            ...
            if val.psi_k < 1e-6:
                continue
            f.write(
                fmt.format(type[0], type[1], type[2], type[3], val.psi_k, val.psi_eq, 1)
            )
...
def writePRM(mol, parameters, filename):
    ...
    f = open(filename, "w")
    print(
        "* prm file built by HTMD parameterize version {}".format(htmdversion()), file=f
    )
    print("*\n", file=f)
...
def writeRTF(mol, parameters, netcharge, filename):
    ...
    f = open(filename, "w")
    print(
        "* Charmm RTF built by HTMD parameterize version {}".format(htmdversion()),
        file=f,
    )
    print("* ", file=f)
    print("  22     0", file=f)
    ...
    maxatomtypelen = max(
        6, np.max([len(at) for at in mol.atomtype])
    )  # Minimum field size of 6
    for n, a, c in zip(mol.name, mol.atomtype, mol.charge):
        print(
            "ATOM {0:>{1}s} {2:>{3}s} {4:8.6f}".format(
                n, maxnamelen, a, maxatomtypelen, c
            ),
            file=f,
        )
    for a in mol.bonds:
        print(
            "BOND {:>4s} {:>4s}".format(*sorted([mol.name[a[0]], mol.name[a[1]]])),
            file=f,
        )
