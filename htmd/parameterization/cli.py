...
def getArgumentParser():
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="List parameterizable dihedral angles"
    )
    parser.add_argument(
        "--rtf-prm", nargs=2, metavar="<filename>", help="CHARMM RTF and PRM files"
    )
    parser.add_argument(
        "-ff",
        "--forcefield",
    ...
    parser.add_argument(
        "--max-jobs", type=int, default=sys.maxsize, help=argparse.SUPPRESS
    )
...
def _prepare_molecule(args):
    ...
    from htmd.parameterization.util import (
        makeAtomNamesUnique,
        guessElements,
        detectChiralCenters,
    )
    ...
    if mol.numFrames != 1:
        raise RuntimeError(
            "{} has to contain only one conformation, but found {}".format(
                args.filename, mol.numFrames
            )
        )
    ...
    if args.charge is None:
        args.charge = charge
        logger.info(
            "Molecular charge is set to {} by adding up the atomic charges in {}"
        ...
        if args.charge_type == "None" and args.charge != charge:
            raise ValueError(
                "The molecular charge is set to {}, but the partial atomic charges in {} "
    ...
    if np.unique(mol.name).size != mol.numAtoms:
        ...
        for i, (old_name, new_name) in enumerate(zip(mol.name, new_mol.name)):
            if old_name != new_name:
                logger.warning(
                    "Renamed atom {:3d}: {:4s} --> {:4s}".format(i, old_name, new_name)
                )
        mol = new_mol
    ...
    if not np.all(mol.resname == mol.resname[0]):
        raise RuntimeError(
            "All atoms of the molecule need to have the same residue name"
        )
...
def _select_dihedrals(mol, args):
    ...
    for i, equivalent_dihedrals in enumerate(parameterizable_dihedrals):
        names = [
            "-".join(mol.name[list(dihedral)]) for dihedral in equivalent_dihedrals
        ]
    ...
    if args.list:
        ...
        with open(dihedral_file, "w") as file:
            for dihedrals in parameterizable_dihedrals:
                ...
                file.write("{}\n".format(name))
            logger.info(
                "Write the list of the parameterizable dihedral angles to {}".format(
                    dihedral_file
                )
            )
        sys.exit()
...
def main_parameterize(arguments=None, progress=None):
    ...
    from htmd.parameterization.parameterset import (
        recreateParameters,
        createMultitermDihedralTypes,
        inventAtomTypes,
    )
    from htmd.parameterization.util import (
        detectChiralCenters,
    ...
    if (
        args.min_type == "qm"
        or args.charge_type == "ESP"
        or (args.fit_dihedral and not args.nnp)
    ):
    ...
    if len(selected_dihedrals) > 0:
        ...
        for name, old_type, new_type in zip(mol.name, old_types, mol.atomtype):
            if old_type != new_type:
                logger.info(
                    "   {:4s} : {:6s} --> {:6s}".format(name, old_type, new_type)
                )
