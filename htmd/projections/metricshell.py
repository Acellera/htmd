...
class MetricShell(MetricDistance):
    ...
    Parameters
    ----------
    sims : numpy.ndarray of :class:`Sim <htmd.simlist.Sim>` objects or single :class:`Molecule <moleculekit.molecule.Molecule>` object
        A simulation list generated by the :func:`simlist <htmd.simlist.simlist>` function, or a :class:`Molecule <moleculekit.molecule.Molecule>` object
    sel1 : str
    ...
    def _precalculate(self, mol):
        self.description = np.vstack(super().getMapping(mol).atomIndexes)
        self.shellcenters = np.unique(self.description[:, 0])
    ...
    def _getPrecalc(self, mol):
        ...
        if self.shellcenters is not None and self.description is not None:
            return self.shellcenters, self.description, sel1, sel2
        else:
