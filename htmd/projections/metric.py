...
class Metric:
    ...
    def set(self, projection):
        ...
        if not (
            isinstance(self.projectionlist, list)
            or isinstance(self.projectionlist, tuple)
        ) and (
            isinstance(self.projectionlist, Projection)
            or hasattr(self.projectionlist, "__call__")
        ):
            self.projectionlist = [
                self.projectionlist,
            ]
        elif isinstance(self.projectionlist, list) or isinstance(
            self.projectionlist, tuple
        ):
            ...
            raise AttributeError(
                "Metric.set only accepts Projection objects, functions, function/argument tuples or "
    ...
    def project(self, njobs=None):
        ...
        results = aprun(total=numSim, desc="Projecting trajectories")(
            delayed(_processSim)(
                self.simulations[i], self.projectionlist, uqMol, self.skip
            )
    ...
    def _projectSingle(self, index):
        data, ref, fstep, _ = _processSim(
            self.simulations[index], self.projectionlist, None, self.skip
        )
    ...
    def _removeEmpty(self, metrics, ref, deletesims, fstep):
        ...
        if len(metrics) == 0:
            raise NameError(
                "No trajectories were projected. Check if the simlist is empty or for projection errors."
            )
...
def _project(proj, target):
    if isinstance(proj, Projection):
        ...
        res = proj(target)
    elif isinstance(proj, tuple) and hasattr(
        proj[0], "__call__"
    ):  # If it's a function with extra arguments
...
def _checkProjectionDims(result, mol, name):
    if (result.ndim == 1 and len(result) != mol.numFrames) or (
        ...
        raise RuntimeError(
            "The {} produced an incorrect result. It produced a {} shaped array for {} frames. "
...
def _processSim(sim, projectionlist, uqmol, skip):
    ...
    try:
        ...
        for proj in projectionlist:
            ...
            if result.size == 0:
                logger.warning(
                    f"No data was produced by projection {proj.__class__} of simulation id: {sim.simid}"
                )
            data.append(result)
...
def _singleMolfile(sims):
    ...
    if isinstance(sims, Molecule):
        ...
        if len(uqmolfiles) == 0:
            ...
            len(uqmolfiles) > 1
        ):  # If more than one molfile load them and see if they are different Molecules
...
def _projectionGenerator(metric, njobs):
    for i in range(0, len(metric.simulations), njobs):
        ...
        results = Parallel(n_jobs=njobs, verbose=0)(
            delayed(_projector)(metric, i) for i in simrange
        )
        yield results
