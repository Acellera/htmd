...
open(FILE, ">FF_" . $Name1 . "_" . $Name2. "_partial.dat");
...
foreach my $Chain (@$Chains) {
  ...
  $AtomTyper->TypeAtomsInChain($Chain);
  ...
  $AtomCharger->ChargeAtomsInChain($CopiedChain);
  ...
  foreach my $MoleculeNum (0 .. @{$CopiedChain->getMolecules}-1) {
    ...
    my %CopiedAtomNames = map { $_->getName => $_ } @{$CopiedChain->getMolecule($MoleculeNum)->getAtoms};

    foreach my $Atom (@{$Chain->getMolecule($MoleculeNum)->getAtoms} ) {
	
	    my $Bonds = $CopiedAtomNames{$Atom->getName}->getBonds;
	  	
	    my $apply_count = 0;
	
	    foreach my $Bond (@$Bonds) {
		
		   $apply_count++ if $Bond->getApplied == 0;
		
	    }  
		 	
	    next if $apply_count != 0;
	    ...
	    if(abs($AtomChargeHash{$Atom->getName} - $CopiedAtomChargeHash{$Atom->getName} ) > 0.01 ) {
	      print $Atom->getName . " " . $Atom->getType . " " . $AtomChargeHash{$Atom->getName} .  " " . $CopiedAtomChargeHash{$Atom->getName} . "\n";
      }
