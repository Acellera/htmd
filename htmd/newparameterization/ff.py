   ...
   for i in self.dihedrals:
     ...
     if not (name in output ):
       ...
       for pi in range(len(prm)): 
         ...
         print( "%s-%s-%s-%s 1 %f %f %f %f %f" % ( t[i.types[0]], t[i.types[1]], t[i.types[2]], t[i.types[3]], p.k0, p.phi0, sign * p.n, scee, scnb ), file=f )
...
class AmberRTF(RTF):
  def __init__( self, mol, prepi, frcmod ):
   ...
   if f[5].strip() != "CORRECT     OMIT DU   BEG" : raise ValueError("Invalid prepi format line 6" )
   ...
   while f[ctr].strip() != "":
     ...
     if not ( ff[2] in self.types ):
        ...
        self.typeindex_by_type[ ff[2] ] = 900 + len(self.types)-1 # add a big offset so it doesn't collide with real charm types
