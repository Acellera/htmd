...
def main_parameterize():
    ...
    parser.add_argument("-n", "--ncpus", help="Number of CPUs to use (default {})".format(ncpus), default=ncpus,
                        dest="ncpus")
    parser.add_argument("-f", "--forcefield", help="Inital FF guess to use", choices=["GAFF", "GAFF2", "CGENFF"],
                        default="CGENFF")
    parser.add_argument("-b", "--basis", help="QM Basis Set", choices=["6-31g-star", "cc-pVTZ"], default="cc-pVTZ",
                        dest="basis")
    parser.add_argument("-e", "--exec", help="How to run the QM", choices=["inline", "LSF", "PBS"], default="inline",
                        dest="exec")
    ...
    mol = FFMolecule(filename=filename, method=method, netcharge=args.charge, rtf=args.rtf, prm=args.prm, basis=basis,
                     execution=execution, qmcode=code)
