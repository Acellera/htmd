...
def simlist(
    ...
    Returns
    -------
    sims : list of :class:`Sim <htmd.simlist.Sim>` objects
...
def simfilter(
    ...
    outfolder: str,
    filtersel: str | np.ndarray,
    njobs: int | None = None,
) -> np.ndarray:
    """Filter a list of simulations generated by :func:`simlist`.

    Writes new trajectories containing only the desired atoms to a new directory.

    Parameters
    ----------
    sims : list or numpy.ndarray
        A simulation list produced by :func:`simlist`.
    outfolder : str
    ...
    filtersel : str or np.ndarray
        ...
        describing the atoms to keep.
    njobs : int, optional
        Number of parallel jobs to spawn for filtering. If None, uses the
        value from htmd.config.
...
def simmerge(
    simlist1: list | np.ndarray, simlist2: list | np.ndarray
) -> list | np.ndarray:
    """Merge two simlists by updating their ``simid`` fields.

    Parameters
    ----------
    simlist1 : list or numpy.ndarray of :class:`Sim <htmd.simlist.Sim>` objects
        First list.
    simlist2 : list or numpy.ndarray of :class:`Sim <htmd.simlist.Sim>` objects
        Second list.

    Returns
    -------
    newlist : list or numpy.ndarray of :class:`Sim <htmd.simlist.Sim>` objects
        All simulations from both input lists with updated ``simid`` values. If
        one input is empty, the other is returned unchanged.
    """
    if len(simlist1) == 0:
        return simlist2
    if len(simlist2) == 0:
