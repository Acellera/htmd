...
def build(mol, ff=None, topo=None, param=None, prefix='structure', outdir='./build', caps=None, ionize=True, saltconc=0,
          ...
          offlibraries=None, strip_hydrogens=True):
    ...
    strip_hdyrogens : bool
        Enable or disable removing of hydrogens before building. tleap will
        error if hydrogens don't conform to amber naming. Default is True.
    ...
    mol = mol.copy()
    if strip_hydrogens:
        logger.info('Removing hydrogens from the molecule (avoids common error in tleap; set strip_hydrogens=False if you do not want this functionality)')
        mol.filter('noh')
