...
def _mol2_to_prepi(mol2f, prepif):
    ext = os.path.splitext(mol2f)[1][1:]
    os.system(f"antechamber -fi {ext} -fo prepi -i {mol2f} -o {prepif} -j 0 -pf y")
...
def _build(
    ...
    with open(os.path.join(outdir, "tleap.in"), "w") as f:
        ...
        for i, fname in enumerate(sorted(topo)):
            ...
            #     outdir, f"topo{i}_{os.path.splitext(os.path.basename(fname))[0]}.prepi"
            # )
            # ext = os.path.splitext(fname)[1]
            # if ext == ".mol2":
            #     _mol2_to_prepi(fname, newname)
            # else:
            #     shutil.copy(fname, newname)
...
class _TestAmberBuild(unittest.TestCase):
    ...
    def test_protein_ligand(self):
        ...
        _ = build(newmol, outdir=tmpdir, param=params, ionize=False)
