    ...
    def _algorithm(self):
        ...
        if self.goalfunction is not None:
            ...
            if len(data.simlist) != len(goaldata.simlist):
                raise RuntimeError(
                    "The goal function was not able to project all trajectories that the MSM projection could. Check for possible errors in the goal function."
                )
        ...
        if self.ticadim > 0:
            ...
            if not self._checkNFrames(datatica):
                return False
        ...
        )  # Recalculate reward for all states
        rewards = self.getRewards(
            rewardtraj, data_q, confstatdist, numstates, self.reward_method
        )
