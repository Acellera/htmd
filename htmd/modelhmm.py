...
class ModelHMM(object):
    """ Constructor for the ModelHMM class. ModelHMM uses PyEMMA [1] internally to calculate Markov models.
    ...
    def maxConnectedLag(self, lags):
        ...
        import pyemma.msm as msm
        itime = msm.its(self.data.St, lags=lags, nits=2).get_timescales()
    ...
    def _defaultLags(self):
        modelen = stats.mode(self.data.trajLengths).mode - 1 # -1 to avoid warnings in timescales calc
        if modelen > 20:
    ...
    def eqDistribution(self, plot=True):
        ...
        """
        self._integrityCheck(postmsm=True)
        macroeq = self.hmm.stationary_distribution
    ...
    def save(self, filename):
        """ Save a :class:`ModelHMM` object to disk
...
def getStateStatistic(model, data, states, statetype='macro', weighted=False, method=np.mean, axis=0, existing=False, target=None):
    ...
    if len(model.data.dat) > 0 and model.data.trajLengths != data.trajLengths:
...
def macroAccumulate(model, microvalue):
    ...
    Parameters
    ----------
    model : :class:`ModelHMM <htmd.model.ModelHMM>` object
