...
class ModelHMM(object):
    ...
    def markovModel(self, lag, macronum):
        ...
        self.hmm = estimate_hidden_markov_model(
            self.data.St.tolist(), macronum, lag, connectivity="largest"
        )
    ...
    def plotTimescales(self, maxlag, numstates):
        ...
        mplt.plot_implied_timescales(
            its, ylog=True, units="ns", dt=self.data.fstep, linewidth=2
        )
    ...
    def eqDistribution(self, plot=True):
        if plot:
            ...
            plt.xticks(
                np.arange(self.macronum) + 0.4, [str(a) for a in self.hmm.active_set]
            )
