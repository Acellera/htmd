...
class Psi4(QMBase):
    ...
    def _writeInput(self, directory, iframe):
        ...
        with open(os.path.join(directory, "psi4.in"), "w") as f:
            ...
            element_basis = [
                self.substituteBasisSet(element, self.basis) for element in elements
            ]
            f.write("basis = ''\n")
    ...
    def _readOutput(self, directory):
        ...
        if os.path.exists(xyzFile):
            with open(xyzFile) as f:
                result.energy = float(
                    f.readline().split()[1]
                )  # Read the 2nd number on the 1st line
        ...
        if self.esp_points is not None:
            ...
            if os.path.exists(espFile):
                ...
                result.esp_values *= (
                    const.angstrom / const.physical_constants["Bohr radius"][0]
                )  # 1/Bohr to 1/Angstrom
            else:
                result.errored = True
